...
void RLPXHandshake::writeAuth() {
  ...
  bytesRef pubk(&m_auth[static_cast<size_t>(Signature::size) + static_cast<size_t>(h256::size)], Public::size);
  bytesRef nonce(&m_auth[static_cast<size_t>(Signature::size) + static_cast<size_t>(h256::size) +
                         static_cast<size_t>(Public::size)],
                 h256::size);

  // E(remote-pubk, S(ecdhe-random, ecdh-shared-secret^nonce) ||
  // H(ecdhe-random-pubk) || pubk || nonce || 0x0)
  Secret staticShared;
  crypto::ecdh::agree(host_ctx_->key_pair.secret(), m_remote, staticShared);
  sign(m_ecdheLocal.secret(), staticShared.makeInsecure() ^ m_nonce).ref().copyTo(sig);
  sha3(m_ecdheLocal.pub().ref(), hepubk);
  host_ctx_->key_pair.pub().ref().copyTo(pubk);
  m_nonce.ref().copyTo(nonce);
  m_auth[m_auth.size() - 1] = 0x0;
  encryptECIES(m_remote, &m_auth, m_authCipher);

  auto self(shared_from_this());
  ba::async_write(m_socket->ref(), ba::buffer(m_authCipher),
                  [this, self](boost::system::error_code ec, std::size_t) { transition(ec); });
}

void RLPXHandshake::writeAck() {
  LOG_ALETH(m_logger) << "ack to";
...
void RLPXHandshake::writeAckEIP8() {
  LOG_ALETH(m_logger) << "EIP-8 ack to";
  RLPStream rlp;
...
void RLPXHandshake::setAuthValues(Signature const& _sig, Public const& _remotePubk, h256 const& _remoteNonce,
                                  uint64_t _remoteVersion) {
  _remotePubk.ref().copyTo(m_remote.ref());
  _remoteNonce.ref().copyTo(m_remoteNonce.ref());
  m_remoteVersion = _remoteVersion;
  Secret sharedSecret;
  crypto::ecdh::agree(host_ctx_->key_pair.secret(), _remotePubk, sharedSecret);
                 ...
                 [this, self](boost::system::error_code ec, std::size_t) {
                   if (ec)
                     ...
                     Public pubk(data.cropped(static_cast<size_t>(Signature::size) + static_cast<size_t>(h256::size),
                                              static_cast<size_t>(Public::size)));
                     h256 nonce(data.cropped(static_cast<size_t>(Signature::size) + static_cast<size_t>(h256::size) +
                                                 static_cast<size_t>(Public::size),
                                             h256::size));
...
void RLPXHandshake::readAuthEIP8() {
  ...
  LOG_ALETH(m_logger) << size << " bytes EIP-8 auth from";
  ...
  ba::async_read(m_socket->ref(), rest, [this, self](boost::system::error_code ec, std::size_t) {
    ...
    if (ec)
      transition(ec);
    else if (decryptECIES(host_ctx_->key_pair.secret(), ct.cropped(0, 2), ct.cropped(2), m_auth)) {
      RLP rlp(m_auth, RLP::ThrowOnFail | RLP::FailIfTooSmall);
      setAuthValues(rlp[0].toHash<Signature>(), rlp[1].toHash<Public>(), rlp[2].toHash<h256>(),
                    rlp[3].toInt<uint64_t>());
      m_nextState = AckAuthEIP8;
      transition();
    } else {
      LOG_ALETH(m_logger) << "EIP-8 auth decrypt failed";
      m_nextState = Error;
...
void RLPXHandshake::readAckEIP8() {
  ...
  LOG_ALETH(m_logger) << size << " bytes EIP-8 ack from";
  ...
  ba::async_read(m_socket->ref(), rest, [this, self](boost::system::error_code ec, std::size_t) {
    ...
    if (ec)
      transition(ec);
    else if (decryptECIES(host_ctx_->key_pair.secret(), ct.cropped(0, 2), ct.cropped(2), m_ack)) {
      ...
      LOG_ALETH(m_logger) << "EIP-8 ack decrypt failed";
      m_failureReason = HandshakeFailureReason::FrameDecryptionFailure;
      m_nextState = Error;
      transition();
    }
...
void RLPXHandshake::error(boost::system::error_code _ech) {
  ...
  LOG_ALETH(m_logger) << errorStream.str();
...
void RLPXHandshake::transition(boost::system::error_code _ech) {
  ...
  if (_ech || m_nextState == Error || m_cancel) {
    if (_ech) m_failureReason = HandshakeFailureReason::TCPError;
    return error(_ech);
  }
  ...
  m_idleTimer.async_wait([this, self](boost::system::error_code const& _ec) {
    if (!_ec) {
      LOG_ALETH(m_logger) << "Disconnecting (Handshake Timeout) from";
  ...
  if (m_nextState == New) {
    ...
    LOG_ALETH(m_logger) << p2pPacketTypeToString(HelloPacket) << " to";
    ...
    s.append((unsigned)HelloPacket).appendList(5)
        << dev::p2p::c_protocolVersion << host_ctx_->client_version << host_ctx_->capability_descriptions
        << host_ctx_->port << host_ctx_->key_pair.pub();
    m_io->writeSingleFramePacket(&s.out(), m_handshakeOutBuffer);
    ...
    ba::async_read(
        m_socket->ref(), boost::asio::buffer(m_handshakeInBuffer, handshakeSizeBytes),
        [this, self](boost::system::error_code ec, std::size_t) {
          if (ec)
            transition(ec);
          else {
            if (!m_io) {
              LOG_ALETH(m_errorLogger) << "Internal error in handshake: RLPXFrameCoder disappeared (" << m_remote << ")";
            ...
            LOG_ALETH(m_logger) << "Frame header from";
            ...
            LOG_ALETH(m_logger) << "Successfully decrypted frame header, validating "
                             "contents...";

            /// check frame size
            bytes const& header = m_handshakeInBuffer;
            uint32_t const frameSize = (uint32_t)(header[2]) | (uint32_t)(header[1]) << 8 | (uint32_t)(header[0]) << 16;
            constexpr size_t expectedFrameSizeBytes = 1024;
            if (frameSize > expectedFrameSizeBytes) {
              // all future frames: 16777216
              LOG_ALETH(m_logger) << "Frame is too large! Expected size: " << expectedFrameSizeBytes
            ...
            /// rlp of header has protocol-type, sequence-id[,
            /// total-packet-size]
            ...
            LOG_ALETH(m_logger) << "Frame header contents validated";
            ...
            ba::async_read(
                ...
                [this, self, headerRLP](boost::system::error_code ec, std::size_t) {
                  ...
                  if (ec)
                    ...
                    if (!m_io) {
                      LOG_ALETH(m_errorLogger) << "Internal error in handshake: "
                    ...
                    LOG_ALETH(m_logger) << "Frame body from";
                    bytesRef frame(&m_handshakeInBuffer);
                    if (!m_io->authAndDecryptFrame(frame)) {
                      LOG_ALETH(m_logger) << "Frame body decrypt failed";
                      m_failureReason = HandshakeFailureReason::FrameDecryptionFailure;
                      m_nextState = Error;
                      transition();
                      return;
                    ...
                    if (packetType != HelloPacket) {
                      LOG_ALETH(m_logger) << "Invalid packet type. Expected: " << p2pPacketTypeToString(HelloPacket)
                                    << ", received: " << p2pPacketTypeToString(packetType);
                      m_failureReason = HandshakeFailureReason::ProtocolError;
                      if (packetType == DisconnectPacket) {
                        try {
                          // Explicitly avoid RLP::FailIfTooLarge exception if
                          // RLP data is smaller than bytes buffer since msg
                          // data is significantly smaller than buffer size
                          RLP rlp{frame.cropped(1), RLP::ThrowOnFail | RLP::FailIfTooSmall};
                          if (!rlp.isNull()) {
                            auto const reason = static_cast<DisconnectReason>(rlp[0].toInt<int>());
                            LOG_ALETH(m_logger) << "Disconnect reason: " << reasonOf(reason);
                          ...
                          LOG_ALETH(m_errorLogger) << "Exception occurred while decoding RLP msg "
                                                "data "
                                                "in "
                    ...
                    LOG_ALETH(m_logger) << p2pPacketTypeToString(HelloPacket) << " verified. Starting session with";
                    try {
                      RLP rlp(frame.cropped(1), RLP::ThrowOnFail | RLP::FailIfTooSmall);
                      host_ctx_->on_success(m_remote, rlp, std::move(m_io), m_socket);
                    } catch (std::exception const& _e) {
                      LOG_ALETH(m_errorLogger) << "Handshake causing an exception: " << _e.what();
...
bool RLPXHandshake::remoteSocketConnected() const {
  return m_socket && m_socket->isConnected() && !m_socket->remoteEndpoint().address().is_unspecified();
}
