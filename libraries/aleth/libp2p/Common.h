...
class NodeIPEndpoint {
 public:
  ...
  explicit operator bool() const { return !m_address.is_unspecified() && m_udpPort > 0 && m_tcpPort > 0; }
...
struct NodeSpec {
  ...
  /// Accepts user-readable strings in the form defined here:
...
class Node {
 public:
  ...
  Node(Public _publicKey, NodeIPEndpoint const& _ip, PeerType _peerType = PeerType::Optional)
      : id(_publicKey), endpoint_(_ip), peerType(_peerType) {}
 ...
 private:
  ...
  mutable std::shared_mutex endpoint_mu_;
...
inline boost::log::formatting_ostream& operator<<(boost::log::formatting_ostream& _strm, Node const& _node) {
  return _strm << _node.id << '@' << _node.get_endpoint();
}

inline boost::log::formatting_ostream& operator<<(boost::log::formatting_ostream& _strm, Node& _node) {
...
inline boost::log::formatting_ostream& operator<<(boost::log::formatting_ostream& _strm,
  ...
  _strm << _peerSessionInfo.id << "|" << _peerSessionInfo.clientVersion << "|" << _peerSessionInfo.host << "|"
        << _peerSessionInfo.port << "|";
  for (auto const& cap : _peerSessionInfo.caps) _strm << "(" << cap.first << "," << cap.second << ")";
  return _strm;
}
...
struct hash<bi::address> {
  size_t operator()(bi::address const& _a) const {
    if (_a.is_v4()) return std::hash<unsigned long>()(_a.to_v4().to_ulong());
    if (_a.is_v6()) {
      auto const& range = _a.to_v6().to_bytes();
      return boost::hash_range(range.begin(), range.end());
    }
    if (_a.is_unspecified())
      return static_cast<size_t>(0x3487194039229152ull);  // Chosen by fair dice roll, guaranteed to be
                                                          // random
