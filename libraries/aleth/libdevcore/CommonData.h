...
/// Converts a templated integer value to the big-endian byte-stream represented
/// on a templated collection. The size of the collection object will be
/// unchanged. If it is too small, it will not represent the value properly, if
/// too big then the additional elements will be zeroed out.
...
/// templated integer value.
/// @a _In will typically be either std::string or bytes.
/// @a T will typically by unsigned, u160, u256 or bigint.
template <class T, class _In>
inline T fromBigEndian(_In const& _bytes) {
  T ret = (T)0;
  for (auto i : _bytes) ret = (T)((ret << 8) | (::byte)(typename std::make_unsigned<decltype(i)>::type)i);
  return ret;
}
...
inline std::string toCompactHex(u256 _val, unsigned _min = 0) { return toHex(toCompactBigEndian(_val, _min)); }
...
inline unsigned bytesRequired(T _i) {
  static_assert(std::is_same<bigint, T>::value || !std::numeric_limits<T>::is_signed,
                "only unsigned types or bigint supported");  // bigint does not carry sign
                                                             // bit on shift
  unsigned i = 0;
  for (; _i != 0; ++i, _i = static_cast<T>(_i >> 8)) {
  }
  return i;
}
...
void trimFront(T& _t, unsigned _elements) {
  static_assert(std::is_standard_layout<typename T::value_type>::value, "");
...
static constexpr std::enable_if_t<!std::is_enum_v<N>, bool> is_integer(Signedness sig) {
  using limits_t = std::numeric_limits<std::decay_t<N>>;
  return limits_t::is_integer &&
         (sig == any || (sig == signed_only && limits_t::is_signed) || (sig != unsigned_only && !limits_t::is_signed));
}

template <typename N>
static constexpr std::enable_if_t<std::is_enum_v<N>, bool> is_integer(Signedness sig) {
  return is_integer<std::underlying_type_t<N>>(sig);
}
